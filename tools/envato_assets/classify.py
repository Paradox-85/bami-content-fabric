...
_LIBRARY_KEYWORD_RULES: list[tuple[re.Pattern[str], str, float]] = [
    (re.compile(r"\bgantt\b", re.I),                          "gantt-matrix",               1.0),
    (re.compile(r"timeline|30-60-90",                re.I),   "historical-timeline",         0.9),
    (re.compile(r"roadmap|milestone",                re.I),   "roadmap-with-milestones",     0.95),
    (re.compile(r"phased.*rollout|rollout.*phase",   re.I),   "phased-rollout-timeline",     0.9),
    (re.compile(r"kpi|dashboard",                    re.I),   "kpi-dashboard-grid",          0.95),
    (re.compile(r"scorecard",                        re.I),   "scorecard",                   0.9),
    (re.compile(r"comparison|competitive\s*matrix",  re.I),   "comparison-table",            0.9),
    (re.compile(r"pros.*cons|advantages.*disadvantages", re.I),"pros-cons-list",             0.95),
    (re.compile(r"before.*after|as[-\s]is.*to[-\s]be", re.I),"before-after-split",          0.9),
    (re.compile(r"pric(e|ing)|tier\s*card",          re.I),   "tier-pricing-cards",          0.95),
    (re.compile(r"\btable\b|data\s*grid",            re.I),   "data-table",                  0.85),
    (re.compile(r"ranking|numbered.*list",           re.I),   "numbered-ranking-list",       0.85),
    (re.compile(r"quote|testimonial",                re.I),   "quote-testimonial-card",      0.95),
    (re.compile(r"team|contact\s*card",              re.I),   "team-contact-card-grid",      0.85),
    (re.compile(r"case\s*study|use\s*case",          re.I),   "case-study-card",             0.9),
    (re.compile(r"checklist|status\s*list",          re.I),   "checklist-status",            0.9),
    (re.compile(r"agenda|toc|table\s*of\s*contents", re.I),   "agenda-toc-list",             0.95),
    (re.compile(r"executive\s*summary",              re.I),   "executive-summary-panel",     0.95),
    (re.compile(r"project\s*charter|charter",        re.I),   "project-overview-card",       0.95),
    (re.compile(r"swot|quadrant|2x2|venn",           re.I),   "quadrant-matrix",             0.85),
    (re.compile(r"process|step|workflow",            re.I),   "numbered-process-steps",      0.75),
    (re.compile(r"circular|loop|cycle|petal",        re.I),   "circular-process-loop",       0.85),
    (re.compile(r"funnel|ladder|narrowing",          re.I),   "funnel-diagram",              0.85),
    (re.compile(r"swimlane|swim\s*lane",             re.I),   "swimlane-diagram",            0.9),
    (re.compile(r"decision\s*tree|flowchart|branching", re.I),"decision-tree-flowchart",     0.9),
    (re.compile(r"mind\s*map|radial|concept\s*map",  re.I),   "mind-map-radial",             0.9),
    (re.compile(r"org\s*chart|hierarchy|reporting",  re.I),   "org-chart",                   0.9),
    (re.compile(r"architecture|system\s*diagram",    re.I),   "architecture-diagram",        0.85),
    (re.compile(r"callout|highlight\s*box",          re.I),   "callout-highlight-box",       0.85),
    (re.compile(r"maturity|capability\s*level",      re.I),   "maturity-model-ladder",       0.85),
    (re.compile(r"impact\s*table|risk.*impact",      re.I),   "impact-table",                0.9),
    (re.compile(r"infographic|3d\s*cube|cube\s*diag",re.I),   "infographic-3d-cube",         0.95),
    # Chart sub-types — specific BEFORE generic catch-all
    (re.compile(r"waterfall|bridge\s*chart|variance",          re.I), "chart-waterfall",       0.93),
    (re.compile(r"box\s*(and|&)?\s*whisker|box\s*plot|violin", re.I), "chart-statistical",     0.93),
    (re.compile(r"sunburst|treemap|tree\s*map",                re.I), "chart-sunburst-treemap",0.93),
    (re.compile(r"donut|pie\s*chart|doughnut",                 re.I), "chart-donut-pie",       0.92),
    (re.compile(r"bar\s*chart|column\s*chart|grouped\s*bar|stacked\s*bar|combo\s*chart",
                                                           re.I), "chart-bar-column",      0.92),
    (re.compile(r"line\s*chart|area\s*chart|trend\s*line",     re.I), "chart-line-area",       0.90),
    (re.compile(r"scatter|bubble\s*chart",                     re.I), "chart-scatter-bubble",  0.90),
    # Generic catch-all (lowest confidence, last resort)
    (re.compile(r"infographic|chart|bar|donut|pie",            re.I), "infographic",           0.50),
    (re.compile(r"icon.*feature|feature.*list",      re.I),   "icon-text-feature-list",      0.85),
    (re.compile(r"multi.*column|narrative",          re.I),   "multi-column-narrative",      0.7),
