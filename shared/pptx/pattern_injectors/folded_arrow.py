"""Native PPTX folded-arrow-horizontal graphical pattern injector.

Deterministic native geometry for ``numbered-process-steps/folded-arrow-horizontal``.
Each step is a branded filled circle with a white numeral, connected by a thin
chevron/folded-arrow to the next step. Title text appears below each circle.

Shape naming convention:
  pattern:numbered-process-steps/folded-arrow-horizontal:step:{idx:02d}:circle
  pattern:numbered-process-steps/folded-arrow-horizontal:step:{idx:02d}:number
  pattern:numbered-process-steps/folded-arrow-horizontal:step:{idx:02d}:title
  pattern:numbered-process-steps/folded-arrow-horizontal:connector:{idx:02d}
"""
...
PATTERN_ID = "numbered-process-steps/folded-arrow-horizontal"
...
@register("folded-arrow-horizontal")
def inject_folded_arrow_horizontal(
    slide: Any,
    tokens: Any,
    x: float = 0.0,
    y: float = 0.0,
    ...
    **params: Any,
    ...
    """Inject the folded-arrow-horizontal native graphical pattern.
    ...
    if not steps:
        raise ValueError("folded-arrow-horizontal: 'steps' parameter is required")
    ...
    col_w = (w - gap * (n - 1)) / n
    circle_d = min(col_w * 0.38, h * 0.4, 0.75)
    ...
    for idx, step in enumerate(steps):
        ...
        style_text_frame(nbox.text_frame, tokens, pt=18, color="white", bold=True, align="CENTER")
        nbox.text_frame.paragraphs[0].runs[0].text = num
        created.append(nbox)

        # --- Folded-arrow connector to next step ---
        if show_connector and idx < n - 1:
            next_cx = x + (idx + 1) * (col_w + gap) + (col_w - circle_d) / 2
            arrow_left = cx + circle_d + 0.05
            arrow_w = next_cx - arrow_left
            arrow_top = cy + circle_d / 2 - 0.06
            arrow_h = 0.12

            # Use a chevron-like arrow: RIGHT_ARROW for a single folded shape
        ...
        if title_text:
            ty = y + circle_d + 0.3
            # Title box fits within the column, clamped to canvas right edge
            title_max_w = min(col_w * 0.75, col_w - 0.4)
            # Ensure right edge doesn't exceed body zone
            expected_left = cx + (col_w - title_max_w) / 2
            max_right = x + w - 0.9  # leave 0.9" margin from body-zone right
