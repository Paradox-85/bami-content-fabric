...
## Revision r3 — Must-fix follow-up blockers from r2 review

Implement all items below before acceptance.

### Blockers to fix
1. **Preserve injected Envato category through `media_library.classify()`**
   - Current bug: `handoff()` invokes `ml.classify.callback()`, and `classify()` blindly re-runs `classify_entry(entry)` on every manifest row, overwriting injected `category/category_source/confidence` for `_envato_ingest` entries.
   - Fix requirement: if an entry already carries an Envato-injected classification (e.g. `category_source == "envato"` or equivalent explicit signal), `media_library.classify()` / `classify_entry()` must preserve it instead of recomputing from filename heuristics.
   - This must work end-to-end through the actual `handoff()` path, not only after `inventory()`.

2. **Add a real handoff integration test**
   - The current test only verifies `ml.inventory.callback()`.
   - Add a test that exercises the real bridge enough to catch the regression above. It must run the relevant `handoff()` path or the equivalent sequence `media_library.inventory -> media_library.classify` with the Envato override in place, and assert that the Envato category remains preserved after classify.

### Important note to address
3. **Make `calibrate()` resilient on reruns**
   - Avoid counting stale sample crops from previous calibration attempts for the same `pack_slug`.
   - Before evaluating the calibration review rate, either clear/replace prior sample crop-index rows for the calibration sample or isolate the sample run under unique ids so repeated calibration does not accumulate stale results.

### Acceptance for revision r3
- Handoff no longer reclassifies Envato-ingest entries away from their injected library category.
- A test exists that would fail on the old behavior and passes on the new one.
- Re-running `calibrate()` does not inflate/contaminate review-rate calculations with stale sample rows.
