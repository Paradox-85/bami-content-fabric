...
|----|----------|-----------|--------|--------|--------|-------|
| E1 | S1 | `shared/pptx/layouts.py` | `_layout_comparison_panel` emitted `kind: "comparison"` not in `BUILDERS`. | `layouts.py:_layout_comparison_panel` | FIXED | Option B: emits `card` blocks instead. Layout now reachable (2026-07-05). |
| E2 | S2 | `shared/pptx/blocks.py` | `add_kpi()` silently dropped `delta` and `period` fields forwarded by `_layout_kpi_strip`. | `blocks.py:add_kpi` → `layouts.py:kpi_strip` | FIXED | Delta rendered as colored trend run; period as caption (2026-07-05). |
